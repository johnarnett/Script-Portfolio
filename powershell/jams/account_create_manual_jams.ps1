...
$LOGFILE = "<<LOG_PATH>>\${date}_create.log"
...
$session = New-PSSession -ConfigurationName Microsoft.Exchange -connectionuri http://ZMVVEXMB01/powershell -Credential $env:username
...
#Loop taken out for manual parameter input
...
# EDIT - 03012016
# Check if username exists, if so add next letter in last name to username
$count = 1
while ($True) {

    $last_initial = $last_name.Substring(0,$count)
    $user = $first_name+$last_initial
    $user = $user.ToLower()
...
write-host "debug 6"
           ...
           Set-ADUser -Identity $user -EmployeeID $salescode
write-host "debug 7"
           ...
           Get-ADUser -Identity $user | Move-ADObject -TargetPath 'OU=ScriptTesting,OU=IT,DC=zumiez,DC=dev'

           # Create Mailbox
           #Enable-Mailbox -Identity $user@zumiez.com -Database $dept
write-host "debug 8"
##################
# Error Checking #
##################
